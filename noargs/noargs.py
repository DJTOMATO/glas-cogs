...
class NoArgs(commands.Cog):
    ...
    async def on_message_without_command(self, message):
        ...
        urls = re.findall(r"(https?://cdn.discordapp.com/\S+)", message.content)
        for url in urls:
            if any(
                ext in url
                for ext in [
                ...
                try:
                    ...
                    if await self.is_channel_whitelisted(channel):
                        ...
                        await channel.send(
                            f"{original_author} posted a file. Here's the cleaned file without extra arguments:\n{clean_url}"
                        )
    ...
    async def whitelist_add(self, ctx, channel: discord.TextChannel):
        ...
        async with self.config.guild(
            ctx.guild
        ).whitelisted_channels() as whitelisted_channels:
    ...
    async def whitelist_remove(self, ctx, channel: discord.TextChannel):
        ...
        async with self.config.guild(
            ...
            if channel.id in whitelisted_channels:
                ...
                await ctx.send(
                    f"{channel.mention} has been removed from the whitelist."
                )
            else:
    ...
    async def whitelist_show(self, ctx):
        ...
        if whitelisted_channels:
            ...
            await ctx.send(
                f"The current whitelisted channels are: {' '.join(channel_mentions)}"
            )
        else:
